...
// ============================================================================
// 文件: GaussianSplatRenderer.cs  
// 功能: 高斯溅射渲染系统 - 核心渲染引擎和编辑工具
// 说明:
//   - GaussianSplatRenderSystem: 单例系统，管理所有高斯溅射对象的渲染
//   - GaussianSplatRenderer: 组件类，代表场景中一个高斯溅射对象
//   关键功能:
//   1. GPU排序: 使用基数排序(Radix Sort)按深度排序溅射点
//   2. 计算视图数据: 将高斯椭球体投影到屏幕空间，计算2D协方差矩阵
//   3. 渲染: 使用计算着色器绘制溅射圆形、调试视图等
//   4. 编辑: 支持选择、移动、旋转、缩放、删除溅射点进行实时编辑
//   5. 导出: 可以将编辑后的数据导出为PLY格式
// ============================================================================
...
{
    // ==================== 高斯溅射渲染系统(单例) ====================
    // 负责管理所有GaussianSplatRenderer对象的渲染，处理合并渲染、排序、合成等
    class GaussianSplatRenderSystem
    {
        ...
        public bool GatherSplatsForCamera(Camera cam)
        {
            ...
            // 2) 先按render order排序，再按相机空间z排序。
            // order大的后绘制，能“压在上层”；同order内按深度控制混合顺序。
        ...
        // ReSharper disable once MemberCanBePrivate.Global - used by HDRP/URP features that are not always compiled
        // 对收集到的对象执行“排序->视图数据计算->绘制”。
        // 返回合成材质，供后续把中间RT合成到相机目标。
        public Material SortAndRenderSplats(Camera cam, CommandBuffer cmb)
        {
            ...
            foreach (var kvp in m_ActiveSplats)
            {
                ...
                // 3) 计算当前相机相关的每溅射视图数据(屏幕轴、颜色等)。
                cmb.BeginSample(s_ProfCalcView);
                gs.CalcViewData(cmb, cam);
                cmb.EndSample(s_ProfCalcView);

                // 4) 根据模式决定拓扑参数并发起DrawProcedural。
                int indexCount = 6;
        ...
        // 获取并清空复用的命令缓冲。
        // Built-in管线中会把它挂到相机BeforeForwardAlpha事件。
        ...
        void OnPreCullCamera(Camera cam)
        {
            ...
            // 仅用于判断是否渲染到backbuffer；该检测方式只在BiRP可用。
    ...
    // ==================== 高斯溅射渲染器组件 ====================
    // 这是一个MonoBehaviour，代表场景中的一个高斯溅射3D对象
    // 绑定到GameObject上，用于渲染和编辑高斯溅射资产
    [ExecuteInEditMode]
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        {
            Splats = 0,              // 正常渲染高斯溅射点(默认)
            DebugPoints = 1,         // 显示为彩色点(用位置和索引着色)
            DebugPointIndices = 2,   // 显示溅射索引号(作为颜色)
            DebugBoxes = 3,          // 显示每个溅射的边界框
            DebugChunkBounds = 4,    // 显示数据块的包围盒
        }
        ...
        // 根据当前资产创建GPU资源：位置/属性/SH缓冲、颜色纹理、chunk缓冲、索引缓冲等。
        void CreateResourcesForAsset()
        {
            ...
            // 3) 有chunk数据则上传真实chunk，否则创建哑元chunk。
            ...
            {
                // 无chunk时创建占位buffer，避免shader侧空引用。
            ...
            // 立方体索引(调试盒子模式用36个索引；普通模式通常只用前6个绘制四边形)。
        ...
        // 计算每个溅射在当前相机下的视图相关数据(屏幕轴、颜色、透明度等)。
        ...
        internal void SortPoints(CommandBuffer cmd, Camera cam, Matrix4x4 matrix)
        {
            ...
            // 2) 调用GPU基数排序，输出重排后的索引缓冲。
            EnsureSorterAndRegister();
            m_Sorter.Dispatch(cmd, m_SorterArgs);
            cmd.EndSample(s_ProfSort);
        }

        // 每帧检查资产或数据哈希是否变化，必要时重建GPU资源。
        public void Update()
        {
        ...
        // 激活资产中记录的第index个相机位姿到MainCamera。
        ...
        // 把“可排序uint编码”还原为float。
        // 用于读取GPU统计出的包围盒min/max。
        ...
        public void EditUpdateSelection(Vector2 rectMin, Vector2 rectMax, Camera cam, bool subtract)
        {
            ...
            // 矩形传入顺序为(xMin, yMax, xMax, yMin)以匹配shader约定。
        ...
        // 导出当前编辑后的溅射数据到目标缓冲。
        // bakeTransform=true时会把对象变换烘焙到导出结果中。
        public bool EditExportData(GraphicsBuffer dstData, bool bakeTransform)
        {
            ...
            // flags用于控制导出时是否应用对象空间到世界空间的变换烘焙。
            if (bakeTransform)
                flags = 1;
        ...
        // 通用计算核派发：根据线程组大小计算dispatch数量并提交命令。
